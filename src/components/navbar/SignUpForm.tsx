    ...
    <>
      {signUpState.data &&
        <div className='w-full h-full flex flex-col gap-4 mt-3'>
          ...
          <button onClick={() => { resetSignUpState(); close() }} className="w-full bg-black text-white p-2 rounded-sm hover:bg-[#6d6d6d]">
            Close
          </button>
        </div>
      }
      <form className={clsx(signUpState.data && 'hidden', "flex flex-col gap-4")}>
        ...
        <div>
          <button onClick={handleSubmit} type="submit" className="w-full bg-black text-white p-2 rounded-sm hover:bg-[#6d6d6d]">
            Sign Up
          </button>
        </div>
        <div className='flex flex-col gap-1'>
          ...
          {signUpState.error && !signUpState.loading && <p className="text-red-500 text-sm">{signUpState.error}</p>}
        </div>
      </form>
    </>
  );
